...
/**
  * Write provisionning data in the NVM.
  * This function is only provided as sample, for test purpose. This has
  * to be replaced by an official procedure during production.
  */
static HAPError sampleProvisionning(void)
{
  HAPError err = kHAPError_None;

  if (HAPPlatformKeyValueStoreSet(&platform.keyValueStore,
                                  kSDKKeyValueStoreDomain_Provisioning,
                                  kSDKKeyValueStoreKey_Provisioning_SetupInfo,
                                  SETUP_INFO, sizeof(*SETUP_INFO)))
  {
    HAPLogError(&kHAPLog_Default, "Error while writing setup info");
    err = kHAPError_Unknown;
  }
  if (HAPPlatformKeyValueStoreSet(&platform.keyValueStore,
                                  kSDKKeyValueStoreDomain_Provisioning,
                                  kSDKKeyValueStoreKey_Provisioning_SetupCode,
                                  SETUP_CODE, sizeof(*SETUP_CODE)))
  {
    HAPLogError(&kHAPLog_Default, "Error while writing setup code");
    err = kHAPError_Unknown;
  }
  if (HAPPlatformKeyValueStoreSet(&platform.keyValueStore,
                                  kSDKKeyValueStoreDomain_Provisioning,
                                  kSDKKeyValueStoreKey_Provisioning_SetupID,
                                  SETUP_ID, sizeof(*SETUP_ID)))
  {
    HAPLogError(&kHAPLog_Default, "Error while writing setup id");
    err = kHAPError_Unknown;
  }
  return err;
}
...
int homekit_main(int argc, char* _Nullable argv[_Nullable]) {
