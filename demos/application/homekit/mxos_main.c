 ...
 * @version V1.0.0
 * @date    05-May-2014
 * @brief   MiCO application entrance, add user application functions and threads.
 ******************************************************************************
...
static const uint8_t verifier[] =
{   0x2C,0x21,0x47,0x4C,0x67,0x0B,0xEF,0x8F,0xA6,0x4A,0xF9,0x61,0x46,0xDA,0xB4,0x30,
...
const uint8_t salt[] =
{   0x8F,0xCC,0xC9,0xC7,0xEA,0x97,0xA7,0x76,0xE5,0xCA,0x9C,0xF0,0x69,0x0C,0x0A,0xAF};
#endif

static merr_t _sensor_read_handler( void* arg );
static merr_t display_refresh( void* arg );
...
merr_t pk_sk_cb( unsigned char *pk, unsigned char *sk, bool write )
{
    merr_t err = kNoErr;
    if ( write == true )
    { //write key to flash
...
merr_t application_start( void *arg )
{
    ...
    hk_init.mfi_cp_port = mxos_I2C_CP;
...
static void HKCharacteristicInit( void )
{
    ...
    err = mxos_system_wlan_get_status( &wlan_status );
    ...
    sprintf( oled_string, "HomeKit %s", MODEL );
    OLED_Clear( );
    OLED_ShowString( 0, 0, oled_string );
    OLED_ShowString( 0, 2, "RGB LED OFF   " );
    OLED_ShowString( 0, 4, "DC MOTOR OFF   " );
    OLED_ShowString( 0, 6, wlan_status->mac );
#endif
...
void user_key1_clicked_callback( void )
{
    ...
    {
        hsb2rgb_led_open( app_context->lightbulb.hue, app_context->lightbulb.saturation,
