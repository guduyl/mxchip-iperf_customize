/**
 ******************************************************************************
 * @file    HomeKitUserInterface.c
 * @author  William Xu
 * @version V1.0.0
 * @date    05-May-2014
 * @brief   This file provide application interface with HomeKit library.
 ******************************************************************************
...
HkStatus HKReadCharacteristicValue( int accessoryID, int serviceID, int characteristicID, value_union *value)
{
  ...
  if(serviceID == 1){
    switch(characteristicID){
      case 1:
        (*value).stringValue = mxos_context->micoSystemConfig.name;
        err = kHKNoErr;
        break;
    }
  }
...
void HKWriteCharacteristicValue(int accessoryID, int serviceID, int characteristicID, value_union value, bool moreComing)
{
  ...
  if(serviceID == 1){
    if(characteristicID == 1)
      strncpy(mxos_context->micoSystemConfig.name, value.stringValue, 64);
  ...
  if(!moreComing){
    ...
    if(app_context->fan.on_status == kHKBusyErr){
      ...
      OLED_ShowString( 0, 4, (app_context->fan.on)?"DC MOTOR ON    ":"DC MOTOR OFF   " );
#endif
    }
  }
  return;
