...
{
    app_log("ble_central_app_handle_dev_state_evt: init state  %d, scan state %d, cause 0x%x",
    ...
    if (ble_central_gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
        if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY)
        {
            ...
            app_log("local bd addr: %02x:%02x:%02x:%02x:%02x:%02x",
                            bt_addr[5],
                            bt_addr[4],
                            bt_addr[3],
                            bt_addr[2],
    ...
    if (ble_central_gap_dev_state.gap_scan_state != new_state.gap_scan_state)
    {
        ...
        {
            app_log("GAP scan stop");
        }
        else if (new_state.gap_scan_state == GAP_SCAN_STATE_SCANNING)
        {
            app_log("GAP scan start");
        }
    }
...
void ble_central_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
    ...
    app_log("ble_central_app_handle_conn_state_evt: conn_id %d, conn_state(%d -> %d), disc_cause 0x%x",
    ...
    case GAP_CONN_STATE_DISCONNECTED:
        {
            ...
            {
                app_log("ble_central_app_handle_conn_state_evt: connection lost, conn_id %d, cause 0x%x", conn_id,
                                 disc_cause);
            }

            app_log("Disconnect conn_id %d", conn_id);
...
{
    app_log("ble_central_app_handle_authen_state_evt:conn_id %d, cause 0x%x", conn_id, cause);

    switch (new_state)
    {
    case GAP_AUTHEN_STATE_STARTED:
        {
            app_log("ble_central_app_handle_authen_state_evt: GAP_AUTHEN_STATE_STARTED");
        }
        break;
...
#endif
                app_log("ble_central_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair success");
...
#endif
                app_log("ble_central_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair failed");
            }
        }
        break;
    ...
    default:
        {
            app_log("ble_central_app_handle_authen_state_evt: unknown newstate %d", new_state);
        }
        break;
    }
}
...
{
    app_log("ble_central_app_handle_conn_mtu_info_evt: conn_id %d, mtu_size %d", conn_id, mtu_size);
}
...
void ble_central_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_SUCCESS:
        {
            ...
            app_log("ble_central_app_handle_conn_param_update_evt update success:conn_id %d, conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x",
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_FAIL:
        {
            app_log("ble_central_app_handle_conn_param_update_evt update failed: conn_id %d, cause 0x%x",
                             conn_id, cause);
        }
        break;
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_PENDING:
        {
            app_log("ble_central_app_handle_conn_param_update_evt update pending: conn_id %d", conn_id);
        }
        break;
...
void mbt_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
    ...
    app_log("ble_central_app_handle_gap_msg: subtype %d", p_gap_msg->subtype);
    ...
    case GAP_MSG_LE_BOND_PASSKEY_DISPLAY:
        {
            ...
            app_log("GAP_MSG_LE_BOND_PASSKEY_DISPLAY: conn_id %d, passkey %ld",
    ...
    case GAP_MSG_LE_BOND_USER_CONFIRMATION:
        {
            ...
            le_bond_get_display_key(conn_id, &display_value);
            app_log("GAP_MSG_LE_BOND_USER_CONFIRMATION: conn_id %d, passkey %ld",
                            conn_id, display_value);
    ...
    case GAP_MSG_LE_BOND_OOB_INPUT:
        {
            ...
            app_log("GAP_MSG_LE_BOND_OOB_INPUT: conn_id %d", conn_id);
    ...
    default:
        app_log("ble_central_app_handle_gap_msg: unknown subtype %d", p_gap_msg->subtype);
        break;
    }
}
...
void ble_central_app_parse_scan_info(T_LE_SCAN_INFO *scan_info)
{
    ...
    while (pos < scan_info->data_len)
    {
        ...
        if ((length > 0x01) && ((pos + length) <= 31))
        {
            ...
            app_log("ble_central_app_parse_scan_info: AD Structure Info: AD type 0x%x, AD Data Length %d", type,
                             length - 1);
...
//                             length - 1);
            ...
            case GAP_ADTYPE_FLAGS:
                {
                    ...
                    app_log("GAP_ADTYPE_FLAGS: 0x%x", flags);
                }
                break;
            ...
            case GAP_ADTYPE_SERVICES_LIST_16BIT:
                {
                    ...
                    while (i >= 2)
                    {
                        app_log("GAP_ADTYPE_16BIT_XXX: 0x%x", *p_uuid++);
                        i -= 2;
                    }
                }
                break;
            ...
            case GAP_ADTYPE_32BIT_COMPLETE:
                {
                    ...
                    while (i >= 4)
                    {
                        app_log("GAP_ADTYPE_32BIT_XXX: 0x%lx", *p_uuid++);
                        i -= 4;
                    }
                }
                break;
            ...
            case GAP_ADTYPE_SERVICES_LIST_128BIT:
                {
                    uint32_t *p_uuid = (uint32_t *)(buffer);
                    app_log("GAP_ADTYPE_128BIT_XXX: 0x%8.8lx%8.8lx%8.8lx%8.8lx",
            ...
            case GAP_ADTYPE_LOCAL_NAME_COMPLETE:
                {
                    app_log("GAP_ADTYPE_LOCAL_NAME_XXX: %s", buffer);
            ...
            case GAP_ADTYPE_POWER_LEVEL:
                {
                    app_log("GAP_ADTYPE_POWER_LEVEL: 0x%x", scan_info->data[pos + 1]);
                }
                break;
            ...
            case GAP_ADTYPE_SLAVE_CONN_INTERVAL_RANGE:
                {
                    ...
                    app_log("GAP_ADTYPE_SLAVE_CONN_INTERVAL_RANGE: 0x%x - 0x%x", *p_min,
                                    *p_max);
                }
                break;
            ...
            case GAP_ADTYPE_SERVICE_DATA:
                {
                    ...
                    app_log("GAP_ADTYPE_SERVICE_DATA: UUID 0x%x, len %d", *p_uuid,
                                    data_len);
                }
                break;
            case GAP_ADTYPE_APPEARANCE:
                {
                    ...
                    app_log("GAP_ADTYPE_APPEARANCE: %d", *p_appearance);
                }
                break;
            ...
            case GAP_ADTYPE_MANUFACTURER_SPECIFIC:
                {
                    ...
                    app_log("GAP_ADTYPE_MANUFACTURER_SPECIFIC: company_id 0x%x, len %d",
            ...
            default:
                {
                    ...
                    {
                        app_log("  AD Data: Unhandled Data = 0x%x", scan_info->data[pos + i]);
...
T_APP_RESULT ble_central_app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
    ...
    case GAP_MSG_LE_CONN_UPDATE_IND:
        app_log("GAP_MSG_LE_CONN_UPDATE_IND: conn_id %d, conn_interval_max 0x%x, conn_interval_min 0x%x, conn_latency 0x%x,supervision_timeout 0x%x",
    ...
    default:
        app_log("ble_central_app_gap_callback: unhandled cb_type 0x%x", cb_type);
        break;
    }
    return result;
}
...
void ble_central_gcs_handle_discovery_result(uint8_t conn_id, T_GCS_DISCOVERY_RESULT discov_result)
{
    ...
    case GCS_ALL_PRIMARY_SRV_DISCOV:
        app_log("conn_id %d, GCS_ALL_PRIMARY_SRV_DISCOV, is_success %d",
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_ALL_SRV_UUID16:
                app_log("ALL SRV UUID16[%d]: service range: 0x%x-0x%x, uuid16 0x%x",
            ...
            case DISC_RESULT_ALL_SRV_UUID128:
                app_log("ALL SRV UUID128[%d]: service range: 0x%x-0x%x",
            ...
            default:
                app_log("Invalid Discovery Result Type!");
                break;
            }
        }
        break;
    ...
    case GCS_BY_UUID128_SRV_DISCOV:
        app_log("conn_id %d, GCS_BY_UUID128_SRV_DISCOV, is_success %d",
                        conn_id, discov_result.is_success);
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_SRV_DATA:
                app_log("SRV DATA[%d]: service range: 0x%x-0x%x",
    ...
    case GCS_BY_UUID_SRV_DISCOV:
        app_log("conn_id %d, GCS_BY_UUID_SRV_DISCOV, is_success %d",
                        conn_id, discov_result.is_success);
    ...
    case GCS_ALL_CHAR_DISCOV:
        app_log("conn_id %d, GCS_ALL_CHAR_DISCOV, is_success %d",
                        conn_id, discov_result.is_success);
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_CHAR_UUID16:
                ...
                app_log("CHAR UUID16[%d]: decl_handle 0x%x, properties 0x%x, value_handle 0x%x, uuid16 0x%x",
                ...
                app_log("properties:indicate %d, read %d, write cmd %d, write %d, notify %d\r\n",
            ...
            case DISC_RESULT_CHAR_UUID128:
                ...
                app_log("CHAR UUID128[%d]:  decl hndl=0x%x, prop=0x%x, value hndl=0x%x",
    ...
    case GCS_BY_UUID_CHAR_DISCOV:
        app_log("conn_id %d, GCS_BY_UUID_CHAR_DISCOV, is_success %d",
                        conn_id, discov_result.is_success);
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_BY_UUID16_CHAR:
                properties = p_result_table->result_data.char_uuid16_disc_data.properties;
                app_log("UUID16 CHAR[%d]: Characteristics by uuid16, decl hndl=0x%x, prop=0x%x, value hndl=0x%x, uuid16=<0x%x>",
                                i, p_result_table->result_data.char_uuid16_disc_data.decl_handle,
                                p_result_table->result_data.char_uuid16_disc_data.properties,
                                p_result_table->result_data.char_uuid16_disc_data.value_handle,
                                p_result_table->result_data.char_uuid16_disc_data.uuid16);
    ...
    case GCS_BY_UUID128_CHAR_DISCOV:
        app_log("conn_id %d, GCS_BY_UUID128_CHAR_DISCOV, is_success %d",
                        conn_id, discov_result.is_success);
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_BY_UUID128_CHAR:
                properties = p_result_table->result_data.char_uuid128_disc_data.properties;
                app_log("UUID128 CHAR[%d]: Characteristics by uuid128, decl hndl=0x%x, prop=0x%x, value hndl=0x%x",
                                i, p_result_table->result_data.char_uuid128_disc_data.decl_handle,
                                p_result_table->result_data.char_uuid128_disc_data.properties,
                                p_result_table->result_data.char_uuid128_disc_data.value_handle);
                app_log("properties:indicate %d, read %d, write cmd %d, write %d, notify %d",
    ...
    case GCS_ALL_CHAR_DESC_DISCOV:
        app_log("conn_id %d, GCS_ALL_CHAR_DESC_DISCOV, is_success %d\r\n",
                        conn_id, discov_result.is_success);
        for (i = 0; i < discov_result.result_num; i++)
        {
            p_result_table = &(discov_result.p_result_table[i]);
            ...
            case DISC_RESULT_CHAR_DESC_UUID16:
                app_log("DESC UUID16[%d]: Descriptors handle=0x%x, uuid16=<0x%x>",
            ...
            case DISC_RESULT_CHAR_DESC_UUID128:
                app_log("DESC UUID128[%d]: Descriptors handle=0x%x",
    ...
    default:
        app_log("Invalid disc type: conn_id %d, discov_type %d",
...
{
    T_APP_RESULT  result = APP_RESULT_SUCCESS;
    app_log("ble_central_gcs_client_callback: client_id %d, conn_id %d",
                    client_id, conn_id);
    if (client_id == ble_central_gcs_client_id)
    {
        ...
        case GCS_CLIENT_CB_TYPE_READ_RESULT:
            app_log("READ RESULT: cause 0x%x, handle 0x%x,  value_len %d",
        ...
        case GCS_CLIENT_CB_TYPE_WRITE_RESULT:
            app_log("WRITE RESULT: cause 0x%x ,handle 0x%x, type %d",
        ...
        case GCS_CLIENT_CB_TYPE_NOTIF_IND:
            ...
            {
                app_log("INDICATION: handle 0x%x, value_size %d",
                                p_gcs_cb_data->cb_content.notif_ind.handle,
                                p_gcs_cb_data->cb_content.notif_ind.value_size);
            }
            else
            {
                app_log("NOTIFICATION: handle 0x%x, value_size %d",
                                p_gcs_cb_data->cb_content.notif_ind.handle,
                                p_gcs_cb_data->cb_content.notif_ind.value_size);
            }
            break;
